...
const ViewClient = ({ isAuthenticated }) => {
    ...
    const onSaveChanges = async (e) => {
        ...
        if (isEditing) {
            ...
            await axios.post(`${process.env.REACT_APP_API_URL}client/${clientId}?_method=PUT`, data, {headers : {'Authorization' : localStorage.getItem('token'), 'Content-Type' : 'multipart/form-data','Allow-Control-Allow-Origin' : '*',}})
                .then(res => {
    ...
    React.useEffect(() => {
        ...
        const getClient = async () => {
            await axios.get(`${process.env.REACT_APP_API_URL}client/${clientId}`, {headers : {'Authorization' : localStorage.getItem('token'), 'Content-Type' : 'multipart/form-data','Allow-Control-Allow-Origin' : '*',}})
                .then(res => {
                    if (isSub) {
                        ...
                        </div>
                        {
                            isEditing ? 
                                <form onSubmit={onSaveChanges}>
                                    <div className="row client-info">
                                        <div className="col-6">
                                            <img src={process.env.REACT_APP_IMG_URL + clientDetail.image} alt={clientDetail.first_name} className="client-img client-edit" />
                                        </div>

                                        <div className="col-6">
                                            <button type="submit" className="edit-btn">{isSaving ? "..." : <i className="fa-solid fa-floppy-disk"></i>}</button>
                                        </div>
                                    </div>
                                    <div className="row client-info">
                                        ...
                                        <div className="col-6 client-form-field">
                                            {
                                                ...
                                                <div>
                                                    ...
                                                    <select className="new-client-text" style={{width: '100%'}} name="region" id="region" onChange={onChangeText} required> 
                                                        ...
                                                        {
                                                            regionsList.map(d => 
                                                                <option key={d.region_code} value={d.region_code} selected={clientDetail.region === d.region_code ? true : false}>{d.region_name}</option>   
                                                            )
                                                        }
                                                    </select>
                                                </div>
                                        ...
                                        <div className="col-6 client-form-field">
                                            {
                                                ...
                                                <div>
                                                    ...
                                                    <select className="new-client-text" style={{width: '100%'}} name="province" id="province" onChange={onChangeText} required> 
                                                        ...
                                                        {
                                                            provincesList.map(d => 
                                                                <option key={d.province_code + d.id} value={d.province_code} selected={clientDetail.province === d.province_code ? true : false}>{d.province_name}</option>   
                                                            )
                                                        }
                                                    </select>
                                                </div>
                                        ...
                                        <div className="col-6 client-form-field">
                                            {
                                                ...
                                                <div>
                                                    ...
                                                    <select className="new-client-text" style={{width: '100%'}} name="city" id="city" onChange={onChangeText} required> 
                                                        ...
                                                        {
                                                            citiesList.map(d => 
                                                                <option key={d.city_code + d.id} value={d.city_code} selected={clientDetail.city === d.city_code ? true : false}>{d.city_name}</option>   
                                                            )
                                                        }
                                                    </select>
                                                </div>
                                        ...
                                        <div className="col-6 client-form-field">
                                            {
                                                ...
                                                <div>
                                                    ...
                                                    <select className="new-client-text" style={{width: '100%'}} name="barangay" id="barangay" onChange={onChangeText} required> 
                                                        ...
                                                        {
                                                            barangaysList.map(d => 
                                                                <option key={d.brgy_code + d.id} value={d.brgy_code} selected={clientDetail.barangay === d.brgy_code ? true : false}>{d.brgy_name}</option>   
                                                            )
                                                        }
                                                    </select>
                                                </div>
                            ...
                            <>
                                <div className="row client-info">
                                    ...
                                    <div className="col-6">
                                        <button onClick={() => setIsEditing(!isEditing)} className="edit-btn"><i className="fas fa-user-edit"></i></button>
                                    </div>
                                </div>
                                <div className="row client-info">
                                    <div className="col-12">
                                        <div className="client-name">{clientDetail.first_name + ' ' + clientDetail.middle_name + ' ' + clientDetail.last_name}{clientDetail.suffix === "N/A" ? "" : clientDetail.suffix === "NA" ? "" : ", " + clientDetail.suffix} <span className="client-gender">{clientDetail.gender === 1 ? <i className="fas fa-mars" style={{'color': 'skyblue'}}></i> : <i className="fas fa-venus" style={{'color': 'pink'}}></i>} <span className="client-id">{clientDetail.client_id}</span></span></div>
                                ...
                                <div className="row client-info">
                                    <div className="col-6">
                                        <i className="fa-brands fa-facebook"></i> {clientDetail.facebook}
                                    </div>
                                    <div className="col-6">
                                        <i className="fa-brands fa-instagram"></i> {clientDetail.instagram}
                                    </div>
                                </div>
